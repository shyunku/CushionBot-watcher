...
class Sessions extends Layer {
  ...
  preprocess(engine) {
    ...
    const { intervalStart: intervalStartFlex, intervalEnd: intervalEndFlex } =
      engine.variables;
    const now = Date.now();
    ...
    const filtered = flatSessions.filter((s) => {
      ...
      const startIncluded =
        filterStart <= s.joinTime && s.joinTime <= filterEnd;
      const endIncluded =
        filterStart <= s.leaveTime && s.leaveTime <= filterEnd;
    ...
    const sortedUsers = Object.values(this.guild.users).sort((a, b) => {
      ...
      const totalDuration1 = sessions1.reduce(
        (acc, s) => acc + (s.endTime - s.joinTime),
        0
      );
      const totalDuration2 = sessions2.reduce(
        (acc, s) => acc + (s.endTime - s.joinTime),
        0
      );
    ...
    const filteredSessions = filtered.map((s) => {
      ...
      const endX = s.online
        ? this.calculateX(now)
        : this.calculateX(s.leaveTime);

      const color = Color.generateRandomColorWithSeed(
        s.channelName ?? "Unknown"
      )
  ...
  draw(c, engine) {
    ...
    const totalSessionCount = sortedUserSessions.reduce(
      (acc, u) => acc + u.sessions.length,
      0
    );
    engine.tool.drawText2(
      `${totalSessionCount} sess, ${parseInt(engine.fps)} fps`,
      5,
      topPad + 5,
      1,
      0,
      ...
      (c) => {
        for (let userSessions of sortedUserSessions) {
          ...
          const y =
            sessionBoxTopPad +
            topPad +
            order * (sessionBoxHeight + sessionBoxMargin) -
            engine.yOffset;
          ...
          for (let session of sessions) {
            ...
            const showUser =
              next == null ||
              next.startX - endX >
                40 + engine.tool.getTextWidth(user.effectiveName);
    ...
    if (hoveredSession) {
      ...
      const height =
        pad * 3 +
        imageRadius +
        lineHeight * (session.online ? segmentCount : segmentCount + 1);
